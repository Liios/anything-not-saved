...
/** Gets the elements that matches a selector AND an inner text. */
function getElementsByInnerText(rootElement, selector, innerText) {
	return [...rootElement.querySelectorAll(selector)].filter(el => el.innerText === innerText);
}
...
function processAryion() {
	const bar = document.querySelector(".func-box");
	if (bar) {
		...
		const func = bar.querySelector(".g-box-header.g-corner-all");
		const sep = document.createElement("span");
		sep.innerHTML = " | ";
		func.appendChild(sep);
		func.appendChild(sabt);
	} else {
		console.warn("Action bar not found.");
	}
}
